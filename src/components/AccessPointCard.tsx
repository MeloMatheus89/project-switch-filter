...
export function AccessPointCard({ accessPoint }: AccessPointCardProps) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden transition-transform hover:scale-102 relative">
      ...
      />
      <div className="p-4">
        <h3 className="text-xl font-semibold text-navy-900 mb-2">{accessPoint.model}</h3>
        <p className="text-gray-600 text-sm mb-4">{accessPoint.description}</p>
        ...
        <div className="mt-3 text-sm text-gray-600">
          <p>
            Frequência: <strong>{accessPoint.frequencyBands.join(", ")}</strong>
          </p>
          <p>
            Banda real Máxima: <strong>{accessPoint.maxThroughput} Mbps</strong>
          </p>
          <p>
            Área de cobertura:{" "}
            <strong>
              {accessPoint.environment === "Indoor"
